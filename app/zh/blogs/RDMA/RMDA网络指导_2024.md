...
# 识别CX4/CX5网卡<a name="ZH-CN_TOPIC_0000002093981148"></a>
...
# 安装MLNX驱动<a name="ZH-CN_TOPIC_0000002094140992"></a>
...
1.  选择下载与OS匹配的驱动包，地址：[https://network.nvidia.com/products/infiniband-drivers/linux/mlnx\_ofed/](https://network.nvidia.com/products/infiniband-drivers/linux/mlnx_ofed/)。
    ...
    ![](./images/zh-cn_image_0000002096008208.png)

2.  新建目录，将OS镜像文件挂载至新建目录。OS镜像名称请根据实际情况进行修改。

    ```
    mkdir -p /mnt/iso
    mount openEuler-22.03-LTS-aarch64-dvd.iso /mnt/iso
    ```
...
# 安装检查<a name="ZH-CN_TOPIC_0000002129740601"></a>
...
4.  查询设备路径和网络端口。
    ...
    2.  查看需要检查的网络端口，后续步骤将对查询到的所有端口进行检查。
        ...
        ![](./images/zh-cn_image_0000002129660297.png)
...
6.  检查固件网络协议。
    1.  查看当前网络协议，此处以ETH协议为例。
        ...
        ![](./images/zh-cn_image_0000002093981160.png)
    ...
    2.  <a name="li7704752131814"></a>查看LINK\_TYPE\_P1、LINK\_TYPE\_P2的值，以/dev/mst/mt4123\_pciconf0为例。
        ...
        -   若查询有结果，表示网络协议可修改。
            -   查询值应为ETH\(2\)，跳至[7](#li71941554151214)。
                ...
                ![](./images/zh-cn_image_0000002094141004.png)

            -   若查询值为IB\(1\)，则执行[6.c](#li1297794616574)修改。

                ![](./images/zh-cn_image_0000002129740609.png)
    ...
    3.  <a name="li1297794616574"></a>设置LINK\_TYPE\_P1和LINK\_TYPE\_P2的值，以/dev/mst/mt4123\_pciconf0为例。
        ...
        ![](./images/zh-cn_image_0000002129660289.png)
...
8.  <a name="li1555794655417"></a>（可选）设置固件选项。
    ...
    1.  查询CX卡固件选项PCI\_WR\_ORDERING的值。
        ...
        ![](./images/zh-cn_image_0000002094141008.png)
    ...
    2.  <a name="li19557114614540"></a>对一张CX5卡两个端口的固件选项PCI\_WR\_ORDERING进行设置，然后执行**reboot**命令重启，环境恢复后，按照[8](#li1555794655417)再次检查一遍，检查是否修改成功。
        ...
        ![](./images/zh-cn_image_0000002129740633.png)

        ```
        mlxconfig -y -d /dev/mst/mt4119_pciconf0.1 s PCI_WR_ORDERING=1
        ```

        ![](./images/zh-cn_image_0000002093981164.png)

# 配置网卡IP地址<a name="ZH-CN_TOPIC_0000002129740605"></a>
...
1.  查看以太网设备和IB设备/端口之间的关联。
    ...
    ![](./images/zh-cn_image_0000002129740637.png)
...
2.  <a name="li1681619318285"></a>查看网卡状态。
    ...
    ![](./images/zh-cn_image_0000002129660301.png)
...
3.  根据环境配置网卡IP地址。以下为添加“/etc/sysconfig/network-scripts/ifcfg-enp24s0f0“config文件方式。使用**systemctl restart network.service**重启应用。
    ...
    ![](./images/zh-cn_image_0000002093981172.png)

    配置完成后，根据[2](#li1681619318285)查看网卡状态。

    ![](./images/zh-cn_image_0000002094141016.png)

# 配置服务器侧RDMA网卡无损特性<a name="ZH-CN_TOPIC_0000002129660277"></a>

配置RDMA网卡无损特性可以提高网络传输的性能和效率，确保数据传输的可靠性和一致性，同时减少CPU的负担。

>![](./public_sys-resources/icon-notice.gif) **须知：** 
>-   如果需要使用RDMA通信协议，则需要在使用HCOM的所有通信节点上执行本章节的操作。不使用RDMA通信协议时，可以不执行本章节的操作。
>-   服务器每次重启后，都需要重新配置RDMA网卡无损特性。本章节以CX网卡为例描述配置RDMA网卡无损特性的详细操作步骤。

1.  <a name="li1919910204286"></a>登录服务器，查询CX5网卡设备net\_card信息。以查询CX5网卡为例。

    ```
    net_card=$(ibdev2netdev | grep mlx5_1 | awk '{print $5}')
    ```

2.  使用[1](#li1919910204286)查询出的net\_card作为参数，配置CX5网卡。
    ...
    >![](./public_sys-resources/icon-note.gif) **说明：** 
    >服务器每次重启后都需要重新执行当前步骤，重新配置CX5。

3.  配置网卡的CNP（Congestion Notification Packet）中的DSCP（Differentiated Services Code Point）字段。

    ```
    echo 48 >/sys/class/net/${net_card}/ecn/roce_np/cnp_dscp
    ```

4.  配置网卡的RoCEv2中的DCQCN（Data Center Quantized Congestion Notification）拥塞控制机制。
...
# 常用IB命令<a name="ZH-CN_TOPIC_0000002094140984"></a>
