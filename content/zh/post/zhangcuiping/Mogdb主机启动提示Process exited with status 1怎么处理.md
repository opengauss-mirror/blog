...
# Mogdb主机启动提示Process exited with status 1怎么处理？

**背景介绍**：

Mogdb主备环境，主机启动提示Process exited with status 1

**报错原因**：

根据提示，显示FATAL:  could not create shared memory segment: Cannot allocate memory，意思是无法分配内存
...
进入数据库安装目录/home/omm12/mogdb/data，打开postgresql.conf, 发现shared_buffers参数值为12GB，改小后， 改为2GB后启动数据库，发现启动成功。
