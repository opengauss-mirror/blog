...
# openGauss备份恢复<a name="ZH-CN_TOPIC_0000001200514121"></a>

## gs\_probackup<a name="section374152918366"></a>
...
目录为 <gs\_probackup的backup\_path\>/wal/
...
-   基于时间点的恢复
    ...
    恢复全量备份 —\> 2 .用户指定了xid/time/lsn. brm进行遍历所有备份,找出最近的备份集通过gs\_probackup进行恢复
...
-   编辑recover.conf —\> 3. 如果没有指定time/lsn/xid不生成recover.conf文件.如果指定了生成recover.conf.
...
-   编辑配置文件（同一台防止端口冲突关闭归档 —\> 4. 是否需要配置postgres.conf文件
...
-   启动实例
    ...
    gs\_ctl start -D /home/omm/a1 —\> 5. 恢复成功进行gs\_ctl start -D 恢复目录
